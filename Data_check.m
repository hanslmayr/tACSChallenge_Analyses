...
sub=['22'];
datpth=['D:\data\L18_P' sub];
cd(datpth);
conds={'Sh';'A';'B';'C'};
fn=dir(['L18_S' sub '_staircase_*.tsv']);
plot_staircase(fn(1,1).name); 
for k=1:length(conds)
    % plot staircase result
    fn=dir(['L18_S' sub '_B*_', conds{k,1}, '*.tsv']);    
    figure;
    pltidx1=1:2:8;
    pltidx2=2:2:8;
    for j=1:numel(fn)
        ...
        subplot(4,2,pltidx1(j));
        plot(time,signal_filt);
        xlim([100 102]);
        title([conds{k} '-run' num2str(j)]);
        subplot(4,2,pltidx2(j))
        plot(f_plot, X_magnitude);
        xlim([1 60]);
        xlabel('Frequency (Hz)');
        ylabel('Magnitude');
        title(['FFT of ' conds{k} 'run-' num2str(j)]);
        grid on;
